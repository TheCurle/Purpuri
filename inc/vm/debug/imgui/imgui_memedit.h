...
#include <stdint.h>     // uint8_t, etc.7
...
struct BytecodeListing
{
    ...
    bool            OptShowAscii;                               // = false  // display ASCII representation on the right side.
    bool            OptShowBytecode;                            // = true   // display Bytecode Translations on the right side.
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        s.BytecodeLine = 0.0f;
        if (OptShowAscii || OptShowBytecode)
        {
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, size_t highlight_addr = 0, size_t base_display_addr = 0x0000)
    {
        ...
        std::vector<size_t>   BytecodeInstructions;
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            if (OptShowBytecode)
            {
                // Draw Bytecode Instruction values
                ImGui::SameLine(s.PosAsciiStart);
                ImVec2 pos = ImGui::GetCursorScreenPos();
                ...
                for (int n = 0; n < Cols && s.BytecodeByte < mem_size; n++) {
                    ...
                    // Retrieve parameters optionally
                    while(instr_length > 1 && mem_data[s.BytecodeByte]) {
                        if (s.BytecodeByte == DataEditingAddr) {
                            ImVec2 posStart(pos.x + (s.GlyphWidth * instr_name.size()), pos.y);
                    ...
                    // Finalise and draw
                    draw_list->AddText(pos, color_text, instr_name.c_str(), instr_name.c_str() + instr_name.size());
        ...
        size_t line = 0;
        for(size_t byte : BytecodeInstructions) {
            ImGui::SetCursorPosY(line * s.LineHeight);
            ImGui::SetCursorPosX(s.PosAsciiStart);
            ImGui::PushID(byte);
            if (ImGui::InvisibleButton("bc", ImVec2(s.PosAsciiEnd - s.PosAsciiStart, s.LineHeight))) {
                DataEditingAddr = DataPreviewAddr = byte;
                DataEditingTakeFocus = true;
            }
            ImGui::PopID();
            line++;
